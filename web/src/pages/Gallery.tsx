...
export function Gallery() {
  ...
  useEffect(() => { api.get<Item[]>("/api/gallery").then(setItems).catch(() => {}); }, []);
  ...
  return (
    <Layout>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        {items.length === 0 && <p className="text-center text-muted">Our gallery is coming soon 💖</p>}
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {shown.map((it) => {
            const embed = it.type === "VIDEO" ? embedUrl(it.url) : null;
            return (
              <div key={it.id} className="lift overflow-hidden rounded-[1.5rem] border border-border bg-surface">
                {it.type === "BEFOREAFTER" ? <BeforeAfter before={it.beforeUrl} after={it.url} />
                  : it.type === "VIDEO" ? (embed
                    ? <div className="aspect-video"><iframe src={embed} title={it.caption} allowFullScreen className="h-full w-full" /></div>
                    : <video src={it.url} controls className="aspect-video w-full bg-black object-cover" />)
                  : <button onClick={() => setLightbox(it.url)} className="group block aspect-square w-full overflow-hidden"><img src={it.url} alt={it.caption} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></button>}
