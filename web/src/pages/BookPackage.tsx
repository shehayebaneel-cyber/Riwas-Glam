...
export function BookPackage({ packageId }: { packageId: number }) {
  ...
  if (!pkg) return <div className="p-16 text-center text-muted">Loading package…</div>;
    ...
    <div className="min-h-screen bg-surface-2">
      <header className="border-b border-border bg-surface">
        <div className="mx-auto flex max-w-3xl items-center px-4 py-3">
          ...
          <Link to="/packages" className="ml-auto text-sm font-semibold text-muted hover:text-ink">← Packages</Link>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="card mb-5 flex items-center gap-4 p-4">
          <div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-brand-soft">{pkg.image ? <img src={pkg.image} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-2xl">🎀</div>}</div>
          <div className="min-w-0">
            <p className="font-display font-bold text-ink">{pkg.title}</p>
            <p className="text-sm text-muted">{durationLabel(pkg.durationMin)} · <span className="font-semibold text-brand">{priceLabel(pkg.price)}</span></p>
          </div>
        </div>
          ...
          {step === 0 && (
            <div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                {staff.map((m) => (
                  <button key={m.id} onClick={() => { setStaffId(m.id); setStep(1); }} className={`card flex items-center gap-3 p-4 text-left transition hover:border-brand ${staffId === m.id ? "border-brand" : ""}`}>
