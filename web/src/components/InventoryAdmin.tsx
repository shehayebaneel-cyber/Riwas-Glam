...
type Summary = {
  ...
  todayUsage: number;
  expiringSoon: number;
  lowItems: { id: number; name: string; quantity: number; minQuantity: number; unit: string }[];
  expiringItems: { id: number; name: string; expiryDate: string }[];
};
...
function Overview({ hdr }: { hdr: Record<string, string> }) {
  ...
  useEffect(() => {
    api
      .get<Summary>("/api/admin/inventory/summary", hdr)
      .then(setS)
    ...
    <div className="space-y-4">
          ...
          <div className="mt-2 space-y-1 text-sm">
            {s.lowItems.map((p) => (
              <div key={p.id} className="flex justify-between">
                ...
                <span className={p.quantity <= 0 ? "font-semibold text-red-500" : "font-semibold text-amber-600"}>
                  {p.quantity} {p.unit} left (min {p.minQuantity})
                </span>
              </div>
            ))}
          </div>
      ...
      {s.expiringItems.length > 0 && (
        <div className="card p-4">
          <p className="font-display text-brand-dark font-bold">Expiring within 30 days</p>
          <div className="mt-2 space-y-1 text-sm">
            {s.expiringItems.map((p) => (
              <div key={p.id} className="flex justify-between">
                <span className="text-ink">{p.name}</span>
                <span className="text-muted">{p.expiryDate}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <p className="text-muted text-center text-xs">
        Email/SMS low-stock alerts need a messaging provider — they'll light up here for now, and can be wired to email later.
      </p>
    </div>
  );
}
...
function Products({ hdr }: { hdr: Record<string, string> }) {
  ...
  const load = () =>
    api
      .get<Product[]>("/api/admin/products", hdr)
      .then(setItems)
      .catch(() => {});
  useEffect(() => {
  ...
  const badge = (p: Product) =>
    p.quantity <= 0
      ...
      : p.quantity <= p.minQuantity
        ? ["Low", "bg-amber-400/15 text-amber-600"]
        : ["In stock", "bg-emerald-500/15 text-emerald-600"];

  async function del(id: number) {
    if (!confirm("Delete this product?")) return;
    await api.delete(`/api/admin/products/${id}`, hdr);
    load();
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {items.length === 0 && <p className="card text-muted p-8 text-center">No products yet. Add your first item.</p>}
      {items.map((p) => {
        const [label, cls] = badge(p);
        return (
          <div key={p.id} className="card p-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-display text-ink font-bold">{p.name}</span>
              {p.brand && <span className="text-muted text-xs">{p.brand}</span>}
              <span className={`ml-auto rounded-full px-2.5 py-0.5 text-[11px] font-bold ${cls}`}>{label}</span>
            </div>
            <div className="text-muted mt-1 flex flex-wrap gap-x-4 gap-y-1 text-sm">
              <span>
                <b className="text-ink">{p.quantity}</b> {p.unit} in stock
              </span>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button onClick={() => setStockFor(p)} className="btn btn-ghost px-3 py-1.5 text-xs text-emerald-600">
                ± Stock
              </button>
...
function ProductForm({ hdr, product, onClose, onSaved }: { hdr: Record<string, string>; product: Product | null; onClose: () => void; onSaved: () => void }) {
  const [f, setF] = useState<Record<string, string>>(
    product ? { ...emptyProduct, ...Object.fromEntries(Object.entries(product).map(([k, v]) => [k, String(v ?? "")])) } : { ...emptyProduct },
  );
  ...
  async function save() {
    ...
    const body = {
      ...
      ...(product ? {} : { quantity: Number(f.quantity) || 0 }),
    };
    try {
  ...
  const F = (k: string, label: string, type = "text") => (
    <label className="block">
      <span className="text-ink mb-1 block text-xs font-semibold">{label}</span>
      <input type={type} value={f[k]} onChange={(e) => set(k, e.target.value)} className="input !py-2 text-sm" />
    </label>
  );
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="bg-surface w-full max-w-lg rounded-t-[1.5rem] p-5 shadow-2xl sm:rounded-[1.5rem]" onClick={(e) => e.stopPropagation()}>
        <p className="font-display text-ink text-lg font-bold">{product ? "Edit product" : "Add product"}</p>
...
function StockModal({ hdr, product, onClose, onSaved }: { hdr: Record<string, string>; product: Product; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      await api.post(`/api/admin/products/${product.id}/stock`, { type, quantity: Number(qty), note }, hdr);
      onSaved();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-surface w-full max-w-sm rounded-[1.5rem] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <p className="font-display text-ink text-lg font-bold">{product.name}</p>
        ...
        <div className="bg-surface-2 mt-3 flex gap-1 rounded-full p-1">
          {[
            ...
            <button
              key={v}
              onClick={() => setType(v)}
              className={`flex-1 rounded-full px-2 py-1.5 text-sm font-semibold ${type === v ? "bg-brand text-white" : "text-muted"}`}
            >
              {l}
            </button>
          ))}
...
function Recipes({ hdr }: { hdr: Record<string, string> }) {
  ...
  useEffect(() => {
    ...
    setSaved(false);
    api
      .get<RecipeItem[]>(`/api/admin/services/${serviceId}/recipe`, hdr)
      .then((r) => setRows(r.map((x) => ({ productId: x.productId, quantity: x.quantity }))))
      .catch(() => setRows([]));
    // eslint-disable-next-line
  }, [serviceId]);

  const estCost = rows.reduce((s, r) => {
    const p = products.find((x) => x.id === r.productId);
    return s + (p ? p.costPrice * r.quantity : 0);
  }, 0);
  async function save() {
    if (!serviceId) return;
    const r = await api.put<{ materialCost: number }>(
      `/api/admin/services/${serviceId}/recipe`,
      { items: rows.filter((x) => x.productId && x.quantity > 0) },
      hdr,
    );
    setCost(r.materialCost);
    setSaved(true);
  }
  ...
  return (
    <div className="space-y-4">
      <p className="card text-muted p-4 text-sm">
        Tell the system which products each service uses. When a booking is marked <b>completed</b>, stock is deducted automatically — and each service's{" "}
        <b>material cost</b> (used in Finances) is filled in for you.
      </p>
      ...
      {serviceId && (
        <div className="card p-4">
          {products.length === 0 ? (
            <p className="text-muted text-sm">Add some products first (Products tab).</p>
          ) : (
            <>
              <div className="space-y-2">
                {rows.map((r, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <select
                      value={r.productId}
                      onChange={(e) => setRows(rows.map((x, j) => (j === i ? { ...x, productId: Number(e.target.value) } : x)))}
                    ...
                    <input
                      ...
                      placeholder="Qty"
                    />
                    <button onClick={() => setRows(rows.filter((_, j) => j !== i))} className="text-lg text-red-500">
                      ✕
                    </button>
                  </div>
                ))}
              </div>
              <button onClick={() => setRows([...rows, { productId: 0, quantity: 1 }])} className="btn btn-ghost mt-2 px-4 py-1.5 text-sm">
                + Add product
              </button>
              <div className="border-border mt-4 flex items-center gap-3 border-t pt-3">
                <span className="text-muted text-sm">
                  Est. material cost: <b className="text-ink">{money(estCost)}</b>
                </span>
                <button onClick={save} className="btn btn-primary ml-auto px-5 py-2">
                  Save recipe
                </button>
              </div>
...
function History({ hdr }: { hdr: Record<string, string> }) {
  ...
  useEffect(() => {
    api
      .get<Movement[]>("/api/admin/movements", hdr)
      .then(setItems)
      .catch(() => {}); /* eslint-disable-next-line */
  }, []);
  const badge = (t: string) =>
    t === "RECEIVE" ? "bg-emerald-500/15 text-emerald-600" : t === "USE" ? "bg-brand-soft text-brand-dark" : "bg-surface-2 text-muted";
  return (
    <div className="card p-4">
      <p className="font-display text-brand-dark font-bold">Stock movements</p>
      <div className="mt-3 space-y-2">
        {items.length === 0 ? (
          <p className="text-muted py-6 text-center">No movements yet.</p>
        ) : (
          items.map((m) => (
            <div key={m.id} className="border-border flex items-center gap-2 border-b pb-2 text-sm last:border-0">
              <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${badge(m.type)}`}>{m.type}</span>
              <span className="text-ink font-semibold">{m.product?.name ?? "—"}</span>
              <span className={m.quantity < 0 ? "text-red-500" : "text-emerald-600"}>
                {m.quantity > 0 ? "+" : ""}
                {m.quantity} {m.product?.unit}
              </span>
              {m.note && <span className="text-muted truncate text-xs">{m.note}</span>}
              <span className="text-muted ml-auto shrink-0 text-xs">{new Date(m.createdAt).toLocaleDateString()}</span>
            </div>
          ))
        )}
      </div>
