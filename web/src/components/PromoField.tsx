...
export function PromoField({
  ...
  async function apply() {
    ...
    setErr("");
    try {
      const r = await api.post<Applied>("/api/promo/validate", { code, amount }, authHeader);
      onApply(r);
      setCode("");
    } catch (e) {
    ...
    return (
      <div className="flex items-center justify-between rounded-xl bg-emerald-500/10 px-3 py-2 text-sm">
        <span className="font-semibold text-emerald-700">
          ✓ {applied.code} — {applied.label} (−{priceLabel(applied.discount)})
        </span>
        <button type="button" onClick={onClear} className="text-xs font-semibold text-red-500">
          Remove
        </button>
      </div>
    );
  return (
    <div>
      <div className="flex gap-2">
        ...
        <button type="button" onClick={apply} disabled={busy} className="btn btn-ghost px-4 disabled:opacity-50">
          {t("Apply")}
        </button>
      </div>
