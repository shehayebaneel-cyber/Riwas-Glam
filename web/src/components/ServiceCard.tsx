...
export function ServiceCard({ service, catName, onClick }: { service: Service; catName: string; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="lift group block w-full overflow-hidden rounded-[1.75rem] border border-border bg-surface text-left" style={{ boxShadow: "0 18px 46px -24px rgba(176,104,127,0.32)" }}>
