...
export function BeforeAfter({ before, after, className = "" }: { before: string; after: string; className?: string }) {
  ...
  return (
    <div className={`relative aspect-[4/3] w-full select-none overflow-hidden rounded-2xl ${className}`}>
      ...
      <img src={before} alt="Before" className="absolute inset-0 h-full w-full object-cover" style={{ clipPath: `inset(0 ${100 - pos}% 0 0)` }} draggable={false} />
      ...
      <div className="pointer-events-none absolute inset-y-0 w-0.5 bg-white shadow" style={{ left: `${pos}%` }}>
        <div className="absolute top-1/2 left-1/2 flex h-8 w-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white text-brand shadow-lg">↔</div>
      </div>
