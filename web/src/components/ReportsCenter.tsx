...
const REPORTS: Record<string, { label: string; range: boolean; build: (r: Range, hdr: Record<string, string>) => Promise<Built> }> = {
  revenue: {
    ...
    build: async (r, hdr) => {
      ...
      }>(`/api/admin/analytics?from=${r.from}&to=${r.to}`, hdr);
      return {
        columns: [
        ...
        summary: [
          ...
          { label: "Net profit", value: money(a.netProfit) },
          { label: "Expenses", value: money(a.expenses) },
          { label: "Appointments", value: String(a.appointments) },
        ],
      };
    },
  },
    ...
    build: async (r, hdr) => {
      ...
      return {
        ...
        summary: [
          ...
          { label: "Total", value: money(sum(items, "amount")) },
        ],
      };
    },
  },
  inventory: {
    ...
    build: async (_r, hdr) => {
      const items = await api.get<
        { name: string; brand: string; category: string; quantity: number; unit: string; minQuantity: number; costPrice: number; expiryDate: string }[]
      >("/api/admin/products", hdr);
      const rows = items.map((p) => ({
        ...p,
        value: Math.round(p.quantity * p.costPrice * 100) / 100,
        status: p.quantity <= 0 ? "Out" : p.quantity <= p.minQuantity ? "Low" : "In stock",
      }));
      return {
        columns: [
          ...
          { key: "costPrice", label: "Cost", money: true },
        ...
        summary: [
          ...
          { label: "Total value", value: money(sum(rows, "value")) },
        ],
      };
    },
  },
  staff: {
    ...
    range: true,
    build: async (r, hdr) => {
      const items = await api.get<{ name: string; role: string; appointments: number; revenue: number; commissionPct: number; commissionEarned: number }[]>(
        `/api/admin/payouts?from=${r.from}&to=${r.to}`,
        hdr,
      );
      return {
        columns: [
        ...
        rows: items,
        summary: [
          { label: "Revenue", value: money(sum(items, "revenue")) },
          { label: "Commission owed", value: money(sum(items, "commissionEarned")) },
        ],
      };
    },
  },
    ...
    build: async (_r, hdr) => {
      ...
      return {
        ...
        summary: [
          ...
          { label: "Total paid", value: money(sum(rows, "total")) },
        ],
      };
    },
  },
  appointments: {
    ...
    build: async (r, hdr) => {
      const items = await api.get<
        { date: string; time: string; serviceName: string; staffName: string; customerName: string; customerPhone: string; price: number; status: string }[]
      >(`/api/admin/appointments?from=${r.from}&to=${r.to}`, hdr);
      ...
      return {
        ...
        rows: items,
        summary: [
          { label: "Appointments", value: String(items.length) },
          { label: "Revenue", value: money(Math.round(rev * 100) / 100) },
        ],
      };
    },
  },
  customers: {
    ...
    range: false,
    build: async (_r, hdr) => {
      const items = await api.get<{ name: string; phone: string; email: string; visits: number; spent: number; lastVisit: string }[]>(
        "/api/admin/customers",
        hdr,
      );
      return {
        ...
        summary: [
          ...
          { label: "Total spent", value: money(sum(items, "spent")) },
        ],
      };
    },
  },
  giftcards: {
    ...
    build: async (_r, hdr) => {
      const d = await api.get<{
        items: { code: string; initialValue: number; balance: number; status: string; purchaserName: string; expiresAt: string | null }[];
        summary: { outstanding: number };
      }>("/api/admin/gift-cards", hdr);
      const rows = d.items.map((c) => ({
        ...
        expiry: c.expiresAt ? new Date(c.expiresAt).toLocaleDateString() : "",
      }));
      return {
        columns: [
          { key: "code", label: "Code" },
          { key: "initialValue", label: "Initial", money: true },
          { key: "balance", label: "Balance", money: true },
          { key: "status", label: "Status" },
          { key: "purchaserName", label: "Purchaser" },
          { key: "expiry", label: "Expiry" },
        ],
        rows,
        summary: [
          { label: "Cards", value: String(rows.length) },
          { label: "Outstanding balance", value: money(d.summary.outstanding) },
        ],
      };
    },
  },
    ...
    <div className="space-y-4">
      <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
        {Object.entries(REPORTS).map(([k, r]) => (
          <button key={k} onClick={() => setKey(k)} className={`chip whitespace-nowrap ${key === k ? "chip-active" : ""}`}>
            {r.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex gap-2">
          <button onClick={exportCSV} disabled={!built} className="btn btn-ghost px-4 py-2 text-sm disabled:opacity-50">
            ⬇ CSV (Excel)
          </button>
          <button onClick={exportPDF} disabled={!built} className="btn btn-primary px-4 py-2 text-sm disabled:opacity-50">
            🖨 PDF
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {built.summary.map((s) => (
            <div key={s.label} className="card p-3 text-center">
              ...
              <p className="font-display text-ink text-lg font-extrabold">{s.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {loading ? (
        <p className="card text-muted p-8 text-center">Loading…</p>
      ) : !built ? (
        <p className="card text-muted p-8 text-center">No data.</p>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {built.rows.length === 0 ? (
                  <tr>
                    <td colSpan={built.columns.length} className="text-muted p-6 text-center">
                      No records.
                    </td>
                  </tr>
                ) : (
