...
export function DayCloseAdmin({ adminKey }: { adminKey: string }) {
  ...
  useEffect(() => {
    ...
    setDw(null);
    api
      .get<Report>(`/api/admin/reports/daily-closing?date=${date}`, H)
      .then(setR)
      .catch(() => {});
    api
      .get<Drawer>(`/api/admin/cash-drawer?date=${date}`, H)
      .then(setDw)
  ...
  async function saveDrawer(close = false) {
    ...
    if (next) {
      setDw(next);
      setSavedDw(true);
      setTimeout(() => setSavedDw(false), 1500);
    }
  }
  ...
  function downloadCSV() {
    ...
    const rows: [string, string | number][] = [
      ...
      ["Whish received", r.whishReceived],
      ["Total received", r.totalReceived],
      ["Gift cards sold", r.giftCardsSold],
      ["Material cost", r.material],
      ["Commissions", r.commission],
      ["Expenses", r.expenses],
      ["Appointments completed", r.appointmentsCompleted],
      ["Cancelled", r.cancelled],
      ["No-shows", r.noShows],
      ["Still booked", r.booked],
      ["Inventory items consumed", r.inventoryConsumed.count],
      ["Inventory value", r.inventoryConsumed.value],
    ...
    a.href = csv;
    a.download = `daily-closing-${r.date}.csv`;
    a.click();
  }
  ...
  return (
    <div className="space-y-4">
      <div className="no-print flex flex-wrap items-center gap-2">
        ...
        <button onClick={downloadCSV} disabled={!r} className="btn btn-ghost px-4 py-2 text-sm disabled:opacity-50">
          Download CSV
        </button>
        <button onClick={() => window.print()} disabled={!r} className="btn btn-primary px-4 py-2 text-sm disabled:opacity-50">
          Print / Save PDF
        </button>
      </div>
        ...
        <div className="printable card p-5">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-muted text-sm">Daily closing report</p>
            </div>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              <p className="text-muted mb-1 text-xs font-bold uppercase tracking-wide">Money</p>
            ...
            <div>
              <p className="text-muted mb-1 text-xs font-bold uppercase tracking-wide">Appointments</p>
          ...
          {r.staffCommissions.length > 0 && (
            <div className="mt-4">
              <p className="text-muted mb-1 text-xs font-bold uppercase tracking-wide">Staff commissions</p>
              {r.staffCommissions.map((s) => (
                <Row key={s.name} k={s.name} v={money(s.amount)} />
              ))}
            </div>
          )}
        </div>
      ...
      {dw && (
        <div className="card p-5">
          <div className="flex items-center justify-between">
            <p className="font-display text-ink text-lg font-bold">Cash drawer</p>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            ...
            <Big
              label="Difference"
              value={money(dw.difference)}
              tone={dw.difference === 0 ? "text-ink" : dw.difference > 0 ? "text-emerald-600" : "text-red-500"}
            />
          </div>
          <p className="text-muted mt-2 text-xs">Expected = opening + cash sales − cash expenses − refunds. Cash sales come from paid cash payments.</p>
          ...
          <div className="mt-3 flex gap-2">
            <button onClick={() => saveDrawer(false)} className="btn btn-ghost px-4 py-2 text-sm">
              Save {savedDw && "✓"}
            </button>
            {!dw.closedAt && (
              <button onClick={() => saveDrawer(true)} className="btn btn-primary px-4 py-2 text-sm">
                Close drawer
              </button>
            )}
          </div>
...
function NumField({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <label className="text-ink text-sm font-semibold">
      {label}
      <input type="number" step="0.01" value={value} onChange={(e) => onChange(Number(e.target.value))} className="input mt-1 !py-2 text-sm" />
    </label>
  );
}
function Big({ label, value, tone = "text-ink" }: { label: string; value: string; tone?: string }) {
  return (
    <div className="bg-surface-2 rounded-xl p-3 text-center">
      ...
      <p className={`font-display text-lg font-extrabold ${tone}`}>{value}</p>
    </div>
  );
}
function Row({ k, v }: { k: string; v: string }) {
  return (
    <div className="border-border flex justify-between border-b py-1.5 text-sm last:border-0">
