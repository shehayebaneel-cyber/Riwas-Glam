      ...
      <div className="bg-surface-2 flex gap-1 rounded-full p-1">
        {(
          ...
          <button
            key={v}
            onClick={() => setView(v)}
            className={`flex-1 rounded-full px-3 py-2 text-sm font-semibold ${view === v ? "bg-brand text-white" : "text-muted"}`}
          >
            {l}
          </button>
        ))}
...
function Current({ hdr, range }: { hdr: Record<string, string>; range: { from: string; to: string } }) {
  ...
  const load = () =>
    api
      .get<Row[]>(`/api/admin/payouts?from=${range.from}&to=${range.to}`, hdr)
      .then((r) => {
        setRows(r);
        setAdj({});
        setPaid({});
  ...
  const totalFor = (r: Row) => {
    const a = getAdj(r.staffId);
    return Math.round((r.commissionEarned + a.bonus + a.tips - a.deduction) * 100) / 100;
  };
  ...
  function exportCSV() {
    ...
    const lines = rows.map((r) => {
      const a = getAdj(r.staffId);
      return [r.name, r.role, r.appointments, r.revenue, r.commissionPct, r.commissionEarned, a.bonus, a.tips, a.deduction, totalFor(r)].join(",");
    });
    const csv = [head.join(","), ...lines].join("\n");
    const url = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    const link = document.createElement("a");
    link.href = url;
    link.download = `payouts_${range.from}_${range.to}.csv`;
    link.click();
    URL.revokeObjectURL(url);
  }

  if (rows.length === 0) return <p className="card text-muted p-8 text-center">No staff yet.</p>;
  return (
    <div className="space-y-3">
      <button onClick={exportCSV} className="btn btn-ghost px-4 py-2 text-sm">
        ⬇ Export CSV (Excel)
      </button>
      {rows.map((r) => {
        const a = getAdj(r.staffId);
        return (
          <div key={r.staffId} className="card p-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-display text-ink font-bold">{r.name}</span>
              <span className="text-brand text-xs">{r.role}</span>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {(["bonus", "tips", "deduction"] as const).map((k) => (
                <label key={k} className="block">
                  <span className="text-muted mb-1 block text-xs font-semibold capitalize">{k}</span>
                  <input
                    type="number"
                    value={a[k] || ""}
                    onChange={(e) => setA(r.staffId, k, Number(e.target.value) || 0)}
                    placeholder="0"
            ...
            <div className="border-border mt-3 flex flex-wrap items-center gap-3 border-t pt-3">
              <span className="text-muted text-sm">
                Total payout: <b className="font-display text-brand text-lg">{money(totalFor(r))}</b>
              </span>
              <div className="ml-auto flex gap-2">
                <button onClick={() => payslip(r)} className="btn btn-ghost px-3 py-1.5 text-xs">
                  🖨 Payslip / PDF
                </button>
                <button onClick={() => pay(r)} disabled={paid[r.staffId]} className="btn btn-primary px-4 py-1.5 text-xs disabled:opacity-50">
                  {paid[r.staffId] ? "Paid" : "Mark as paid"}
                </button>
              </div>
            </div>
          </div>
...
function History({ hdr }: { hdr: Record<string, string> }) {
  ...
  const load = () =>
    api
      .get<Payout[]>("/api/admin/payouts/history", hdr)
      .then(setItems)
      .catch(() => {});
  useEffect(() => {
    load(); /* eslint-disable-next-line */
  ...
  async function del(id: number) {
    ...
    await api.delete(`/api/admin/payouts/${id}`, hdr);
    load();
  }
  ...
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between">
        <p className="font-display text-brand-dark font-bold">Payout history</p>
        <p className="text-muted text-sm">
          Total paid: <b className="text-ink">{money(total)}</b>
        </p>
      </div>
      <div className="mt-3 space-y-2">
        {items.length === 0 ? (
          <p className="text-muted py-6 text-center">No payouts recorded yet.</p>
        ) : (
          items.map((p) => (
            <div key={p.id} className="border-border flex flex-wrap items-center gap-2 border-b pb-2 text-sm last:border-0">
              <span className="text-ink font-semibold">{p.staff?.name ?? "—"}</span>
              <span className="text-muted text-xs">
                {p.periodFrom} → {p.periodTo}
              </span>
              <span className="text-muted text-xs">· {new Date(p.paidAt).toLocaleDateString()}</span>
              <span className="font-display text-brand ml-auto font-bold">{money(p.total)}</span>
              <button onClick={() => del(p.id)} className="text-xs font-semibold text-red-500">
                Delete
              </button>
            </div>
          ))
