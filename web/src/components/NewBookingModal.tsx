...
export function NewBookingModal({ adminKey, onClose, onCreated, defaultDate, defaultTime, defaultStaffId }: { adminKey: string; onClose: () => void; onCreated: () => void; defaultDate?: string; defaultTime?: string; defaultStaffId?: string }) {
  ...
  async function save() {
    ...
    try { await api.post("/api/admin/appointments/new", body, hdr); onCreated(); }
    catch (e) { setErr(e instanceof Error ? e.message : "Couldn't create the booking."); } finally { setBusy(false); }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-t-[1.5rem] bg-surface p-5 shadow-2xl sm:rounded-[1.5rem]" onClick={(e) => e.stopPropagation()}>
        <p className="font-display text-lg font-bold text-ink">New booking (phone / walk-in)</p>

        <div className="mt-3 flex gap-1 rounded-full bg-surface-2 p-1">
          {[["service", "Service"], ["package", "Package"]].map(([v, l]) => (
            <button key={v} onClick={() => set("kind", v)} className={`flex-1 rounded-full px-3 py-1.5 text-sm font-semibold ${f.kind === v ? "bg-brand text-white" : "text-muted"}`}>{l}</button>
          ))}
        </div>
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="block"><span className="mb-1 block text-xs font-semibold text-muted">Date</span><input type="date" value={f.date} onChange={(e) => set("date", e.target.value)} className="input" /></label>
            <label className="block"><span className="mb-1 block text-xs font-semibold text-muted">Time</span><input type="time" value={f.time} onChange={(e) => set("time", e.target.value)} className="input" /></label>
          </div>
        ...
        <div className="mt-4 flex gap-2">
          <button onClick={save} disabled={busy} className="btn btn-primary flex-1 py-2.5 disabled:opacity-60">{busy ? "Saving…" : "Create booking"}</button>
