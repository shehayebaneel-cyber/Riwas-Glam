...
type Analytics = {
  revenue: number; material: number; commission: number; expenses: number;
  grossProfit: number; netProfit: number; appointments: number; avgTicket: number;
  bestService: string; topStaff: string;
  ...
  return (
    <div className="space-y-4">
      <div className="no-scrollbar flex gap-1 overflow-x-auto rounded-full bg-surface-2 p-1">
        {([["dashboard", "📊 Dashboard"], ["expenses", "🧾 Expenses"], ["costs", "🧴 Material costs"]] as const).map(([v, l]) => (
...
function Dashboard({ hdr, range }: { hdr: Record<string, string>; range: { from: string; to: string } }) {
  ...
  useEffect(() => {
    setLoading(true);
    api.get<Analytics>(`/api/admin/analytics?from=${range.from}&to=${range.to}`, hdr).then(setA).catch(() => setA(null)).finally(() => setLoading(false));
    // eslint-disable-next-line
  }, [range.from, range.to]);

  if (loading) return <p className="card p-8 text-center text-muted">Crunching numbers…</p>;
  if (!a) return <p className="card p-8 text-center text-muted">No data for this period.</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-5">
        <p className="font-display font-bold text-brand-dark">How profit is calculated</p>
        <div className="mt-3 space-y-1.5 text-sm">
          ...
          <div className="mt-1 border-t border-border pt-2"><Line k="= Net profit" v={money(a.netProfit)} bold tone={a.netProfit >= 0 ? "good" : "bad"} /></div>
        </div>
      </div>
      ...
      {a.daily.length > 0 && (
        <div className="card p-5">
          <p className="font-display font-bold text-brand-dark">Revenue by day</p>
          <div className="mt-4 flex h-40 items-end gap-1">
            {a.daily.map((d) => (
              <div key={d.date} className="group relative flex-1" title={`${d.date}: ${money(d.value)}`}>
                <div className="w-full rounded-t bg-brand transition-all group-hover:bg-brand-dark" style={{ height: `${(d.value / maxDay) * 100}%` }} />
              </div>
            ))}
          </div>
          <p className="mt-2 text-center text-xs text-muted">{a.daily[0].date} → {a.daily[a.daily.length - 1].date}</p>
        </div>
      )}
      ...
      {a.byService.length > 0 && (
        <div className="card overflow-hidden p-0">
          <p className="border-b border-border p-4 font-display font-bold text-brand-dark">Profit by service</p>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-xs uppercase text-muted"><th className="p-3">Service</th><th className="p-3 text-right"># </th><th className="p-3 text-right">Revenue</th><th className="p-3 text-right">Profit</th></tr></thead>
              <tbody>
                {a.byService.map((s) => (
                  <tr key={s.name} className="border-t border-border">
                    <td className="p-3 font-semibold text-ink">{s.name}</td>
                    <td className="p-3 text-right text-muted">{s.count}</td>
...
function Expenses({ hdr, range }: { hdr: Record<string, string>; range: { from: string; to: string } }) {
  ...
  const load = () => api.get<Expense[]>(`/api/admin/expenses?from=${range.from}&to=${range.to}`, hdr).then(setItems).catch(() => {});
  ...
  async function add() {
    ...
    setForm({ ...form, label: "", amount: "", note: "" }); load();
  }
  async function del(id: number) { await api.delete(`/api/admin/expenses/${id}`, hdr); load(); }

  return (
    <div className="space-y-4">
      <div className="card p-4">
        <p className="font-display font-bold text-brand-dark">Add an expense</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="input">{EXPENSE_CATS.map((c) => <option key={c}>{c}</option>)}</select>
          <input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="Description (e.g. July rent)" className="input" />
        ...
        <button onClick={add} className="btn btn-primary mt-3 px-5 py-2">+ Add expense</button>
      </div>

      <div className="card p-4">
        <div className="flex items-center justify-between">
          <p className="font-display font-bold text-brand-dark">Expenses this period</p>
          <p className="font-display text-lg font-extrabold text-ink">{money(total)}</p>
        </div>
        <div className="mt-3 space-y-2">
          {items.length === 0 ? <p className="py-6 text-center text-muted">No expenses recorded.</p> : items.map((e) => (
            <div key={e.id} className="flex items-center gap-3 border-b border-border pb-2 text-sm last:border-0">
              <span className="rounded-full bg-surface-2 px-2 py-0.5 text-xs font-semibold text-muted">{e.category}</span>
              <span className="font-semibold text-ink">{e.label || "—"}</span>
              <span className="text-xs text-muted">{e.date}</span>
              <span className="ml-auto font-semibold text-ink">{money(e.amount)}</span>
              <button onClick={() => del(e.id)} className="text-xs font-semibold text-red-500">Delete</button>
            </div>
          ))}
        </div>
      </div>
...
function MaterialCosts({ hdr }: { hdr: Record<string, string> }) {
  ...
  useEffect(() => { api.get<Cat[]>("/api/admin/catalog", hdr).then(setCats).catch(() => {}); /* eslint-disable-next-line */ }, []);
  ...
  async function save(id: number, materialCost: number) {
    ...
    setSaved(id); setTimeout(() => setSaved((x) => (x === id ? null : x)), 1200);
  }

  return (
    <div className="space-y-4">
      <p className="card p-4 text-sm text-muted">Set the estimated product/material cost for each service (foundation, gel, wax…). This feeds the <b>profit</b> figures. Later, the Inventory module can fill these in automatically.</p>
      {cats.map((c) => (
        <div key={c.id} className="card p-4">
          <p className="font-display font-bold text-brand-dark">{c.emoji} {c.name}</p>
          <div className="mt-2 space-y-1.5">
            {c.services.map((s) => (
              <div key={s.id} className="flex items-center gap-3 text-sm">
                <span className="flex-1 font-semibold text-ink">{s.name}</span>
                <span className="text-xs text-muted">Price {money(s.price)}</span>
                <span className="flex items-center gap-1">
                  <span className="text-xs text-muted">Cost $</span>
                  <input type="number" defaultValue={s.materialCost} onBlur={(e) => save(s.id, Math.max(0, Number(e.target.value) || 0))} className="input !w-24 !py-1.5 text-sm" />
                </span>
...
function Card({ label, value, tone = "ink", big = false }: { label: string; value: string; tone?: "ink" | "good" | "bad"; big?: boolean }) {
  ...
  return <div className={`card p-4 ${big ? "ring-2 ring-brand/30" : ""}`}><p className="text-xs text-muted">{label}</p><p className={`font-display text-2xl font-extrabold ${color}`}>{value}</p></div>;
}
function MiniStat({ label, value }: { label: string; value: string }) {
  return <div className="card p-4"><p className="text-xs text-muted">{label}</p><p className="mt-0.5 font-display text-lg font-bold text-ink">{value}</p></div>;
}
function Line({ k, v, muted = false, bold = false, tone }: { k: string; v: string; muted?: boolean; bold?: boolean; tone?: "good" | "bad" }) {
  ...
  return <div className="flex justify-between"><span className={muted ? "text-muted" : "text-ink"}>{k}</span><span className={`${color} ${bold ? "font-bold" : "font-semibold"}`}>{v}</span></div>;
}
function BarList({ title, data }: { title: string; data: { name: string; value: number }[] }) {
  ...
  return (
    <div className="card p-5">
      ...
      <div className="mt-3 space-y-2">
        {data.length === 0 ? <p className="text-sm text-muted">No data.</p> : data.map((d, i) => (
          <div key={d.name}>
            <div className="flex justify-between text-sm"><span className="text-ink">{d.name}</span><span className="font-semibold text-muted">{money(d.value)}</span></div>
...
function Donut({ title, data }: { title: string; data: { name: string; value: number }[] }) {
  ...
  let acc = 0;
  const stops = data.map((d, i) => { const from = (acc / total) * 100; acc += d.value; const to = (acc / total) * 100; return `${COLORS[i % COLORS.length]} ${from}% ${to}%`; }).join(", ");
  return (
    <div className="card p-5">
      <p className="font-display font-bold text-brand-dark">{title}</p>
      {data.length === 0 ? <p className="mt-3 text-sm text-muted">No data.</p> : (
        <div className="mt-4 flex items-center gap-5">
          <div className="relative h-32 w-32 shrink-0 rounded-full" style={{ background: `conic-gradient(${stops})` }}><div className="absolute inset-[24%] rounded-full bg-surface" /></div>
          <div className="min-w-0 flex-1 space-y-1">
            {data.map((d, i) => (
              <div key={d.name} className="flex items-center gap-2 text-sm">
                ...
                <span className="ml-auto font-semibold text-muted">{Math.round((d.value / total) * 100)}%</span>
              </div>
            ))}
          </div>
        </div>
