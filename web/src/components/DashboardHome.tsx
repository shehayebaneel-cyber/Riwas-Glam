...
type Dash = {
  ...
  charts: {
    ...
    bookingsByStaff: { name: string; value: number }[];
    profitByMonth: { name: string; value: number }[];
  };
};
...
export function DashboardHome({ adminKey, go }: { adminKey: string; go: (tab: string) => void }) {
  ...
  useEffect(() => {
    api
      .get<Dash>("/api/admin/dashboard", { "x-admin-key": adminKey })
      .then(setD)
      .catch(() => setD(null))
      .finally(() => setLoading(false)); /* eslint-disable-next-line */
  }, []);
  if (loading) return <p className="card text-muted p-10 text-center">Loading your dashboard…</p>;
  if (!d) return <p className="card text-muted p-10 text-center">Couldn't load the dashboard.</p>;
    ...
    <div className="space-y-7">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <Tile
            ...
            label="Waiting list"
            value={d.waitlist}
            sub={d.waitlist ? "in the queue" : "empty"}
            tone={d.waitlist ? "brand" : "ink"}
            onClick={() => go("waitlist")}
          />
          <Tile
            icon="📝"
            ...
            sub={d.pendingReviews ? "tap to moderate" : "all clear"}
            tone={d.pendingReviews ? "amber" : "ink"}
            onClick={() => go("reviews")}
          />
          <Tile
            icon="📦"
            ...
            sub={d.lowStock.items.length ? d.lowStock.items.map((i) => i.name).join(", ") : "all stocked"}
            tone={d.lowStock.count ? "red" : "ink"}
            onClick={() => go("inventory")}
          />
          <Tile
            icon="🎂"
            label="Birthdays this month"
            value={d.birthdays.length}
            sub={d.birthdays.length ? d.birthdays.join(", ") : "none"}
            tone={d.birthdays.length ? "brand" : "ink"}
      ...
      <section>
        ...
        <div className="card p-4">
          {d.workingToday.length === 0 ? (
            <p className="text-muted text-sm">Nobody scheduled today.</p>
          ) : (
            <div className="flex flex-wrap gap-1.5">
              {d.workingToday.map((s) => (
                <span key={s.name} className="bg-brand-soft text-brand-dark rounded-full px-3 py-1 text-xs font-semibold">
                  {s.name}
                </span>
              ))}
            </div>
        ...
        <div className="space-y-3">
          {d.charts.revenueByDay.length > 0 && (
            <div className="card p-5">
              <p className="font-display text-brand-dark font-bold">Revenue this month</p>
              <div className="mt-4 flex h-36 items-end gap-1">
                {d.charts.revenueByDay.map((x) => (
                  <div
                    key={x.date}
                    className="bg-brand flex-1 rounded-t"
                    style={{ height: `${(x.value / maxDay) * 100}%` }}
                    title={`${x.date}: ${money(x.value)}`}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <div className="card p-5">
              <p className="font-display text-brand-dark font-bold">Profit by month</p>
              <div className="mt-3 space-y-2">
                {d.charts.profitByMonth.map((m) => (
                  <div key={m.name}>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink">{m.name}</span>
                      <span className={`font-semibold ${m.value >= 0 ? "text-emerald-600" : "text-red-500"}`}>{money(m.value)}</span>
                    </div>
                    <div className="bg-surface-2 mt-1 h-2 rounded-full">
                      <div
                        className={`h-2 rounded-full ${m.value >= 0 ? "bg-emerald-400" : "bg-red-400"}`}
                        style={{ width: `${(Math.abs(m.value) / maxPM) * 100}%` }}
                      />
                    </div>
                  </div>
                ))}
...
function SectionTitle({ children }: { children: string }) {
  return <h2 className="text-muted/70 mb-2.5 text-[11px] font-bold uppercase tracking-[0.14em]">{children}</h2>;
}
function Tile({
  icon,
  label,
          ...
          data.map((d, i) => (
            <div key={d.name}>
              <div className="flex justify-between text-sm">
                ...
                <span className="text-muted font-semibold">{unit === "$" ? money(d.value) : d.value}</span>
              </div>
              <div className="bg-surface-2 mt-1 h-2 rounded-full">
...
function Donut({ title, data }: { title: string; data: { name: string; value: number }[] }) {
  ...
  const stops = data
    .map((d, i) => {
      const f = (acc / total) * 100;
      acc += d.value;
      return `${COLORS[i % COLORS.length]} ${f}% ${(acc / total) * 100}%`;
    })
    .join(", ");
  return (
    <div className="card p-5">
      <p className="font-display text-brand-dark font-bold">{title}</p>
      {data.length === 0 ? (
        <p className="text-muted mt-3 text-sm">No data yet.</p>
      ) : (
          ...
          <div className="min-w-0 flex-1 space-y-1">
            {data.map((d, i) => (
              <div key={d.name} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-ink truncate">{d.name}</span>
                <span className="text-muted ml-auto font-semibold">{money(d.value)}</span>
              </div>
            ))}
          </div>
        </div>
