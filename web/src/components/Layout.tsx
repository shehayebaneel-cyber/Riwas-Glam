...
function Nav() {
  ...
  return (
    <header className={`sticky top-0 z-40 transition-all duration-300 ${scrolled ? "border-b border-border/60 bg-surface/75 shadow-[0_14px_40px_-24px_rgba(176,104,127,0.6)] backdrop-blur-xl" : "border-b border-transparent bg-surface/85 backdrop-blur-md"}`}>
      <div className={`mx-auto grid max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-4 px-5 transition-all duration-300 sm:px-8 ${scrolled ? "h-20 sm:h-32" : "h-[5.5rem] sm:h-[10.5rem]"}`}>
        {/* Logo */}
        <Link to="/" className="flex shrink-0 items-center pe-3 ps-1" aria-label={SITE.name}>
          {SITE.logo
            ? <img src={SITE.logo} alt={SITE.name} className={`w-auto drop-shadow-[0_1px_2px_rgba(74,51,48,0.28)] transition-all duration-300 ${scrolled ? "h-16 sm:h-28" : "h-[4.5rem] sm:h-[9.5rem]"}`} />
