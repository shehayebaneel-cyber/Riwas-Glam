...
export function WebAnalytics({ adminKey }: { adminKey: string }) {
  ...
  useEffect(() => {
    api
      .get<Data>(`/api/admin/analytics/web?days=${days}`, { "x-admin-key": adminKey })
      .then(setD)
      .catch(() => {});
  }, [days, adminKey]);
  if (!d) return null;
  return (
    <div className="card mb-4 p-4">
      <div className="flex items-center justify-between">
        <p className="font-display text-ink font-bold">🌐 Website analytics</p>
...
function List({ title, items }: { title: string; items: L[] }) {
  ...
  return (
    <div>
      <p className="text-muted mb-1 text-xs font-bold uppercase tracking-wide">{title}</p>
      {items.map((i) => (
        <div key={i.label} className="border-border flex justify-between border-b py-1 text-sm last:border-0">
          <span className="text-ink min-w-0 truncate">{i.label}</span>
          <span className="text-muted shrink-0 font-semibold">{i.count}</span>
        </div>
      ))}
    </div>
  );
