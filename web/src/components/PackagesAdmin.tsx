...
export function PackagesAdmin({ adminKey }: { adminKey: string }) {
  ...
  const load = () =>
    api
      .get<Pkg[]>("/api/admin/packages", hdr)
      .then(setItems)
      .catch(() => {});
  useEffect(() => {
    load();
  ...
  async function del(id: number) {
    ...
    await api.delete(`/api/admin/packages/${id}`, hdr);
    load();
  }
  async function toggle(p: Pkg) {
    await api.patch(`/api/admin/packages/${p.id}`, { isActive: !p.isActive }, hdr);
    load();
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {items.length === 0 && <p className="card text-muted p-8 text-center">No packages yet.</p>}
      {items.map((p) => (
        <div key={p.id} className="card flex flex-wrap items-center gap-3 p-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-display text-ink font-bold">
              {p.title} {!p.isActive && <span className="text-muted text-xs font-normal">(hidden)</span>}
            </p>
            <p className="text-muted text-xs">
              {priceLabel(p.price)} · {durationLabel(p.durationMin)} · {p.services.join(", ") || "no services"}
            </p>
          </div>
...
function PackageForm({
  ...
  const [f, setF] = useState({
    ...
    durationMin: String(pkg?.durationMin ?? "60"),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="bg-surface w-full max-w-lg rounded-t-[1.5rem] p-5 shadow-2xl sm:rounded-[1.5rem]" onClick={(e) => e.stopPropagation()}>
        <p className="font-display text-ink text-lg font-bold">{pkg ? "Edit package" : "Add package"}</p>
        <div className="mt-3 space-y-3">
          <label className="block">
            <span className="text-ink mb-1 block text-xs font-semibold">Package name</span>
            <input value={f.title} onChange={(e) => set("title", e.target.value)} className="input" />
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-ink mb-1 block text-xs font-semibold">Total price $</span>
              <input type="number" value={f.price} onChange={(e) => set("price", e.target.value)} className="input" />
            </label>
            <label className="block">
              <span className="text-ink mb-1 block text-xs font-semibold">Duration (min)</span>
              <input type="number" value={f.durationMin} onChange={(e) => set("durationMin", e.target.value)} className="input" />
            </label>
          </div>
          <label className="block">
          ...
          <div>
            <span className="text-ink mb-1 block text-xs font-semibold">Included services</span>
            <div className="border-border max-h-48 space-y-2 overflow-y-auto rounded-xl border p-3">
              {cats.map((c) => (
                <div key={c.id}>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    {c.services.map((s) => (
                      <button key={s.id} type="button" onClick={() => toggle(s.id)} className={`chip !py-1 !text-xs ${ids.has(s.id) ? "chip-active" : ""}`}>
                        {ids.has(s.id) ? "✓ " : ""}
                        {s.name}
                      </button>
                    ))}
          ...
          <div>
            <span className="text-ink mb-1 block text-xs font-semibold">Package image</span>
