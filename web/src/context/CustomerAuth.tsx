...
export function CustomerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.get<Customer>("/api/customer/me", { "x-customer-token": token })
      .then(setCustomer)
      .catch(() => { localStorage.removeItem(TKEY); setToken(""); setCustomer(null); })
      .finally(() => setLoading(false));
  }, [token]);

  const setSession = (t: string, c: Customer) => { localStorage.setItem(TKEY, t); setToken(t); setCustomer(c); };
  const logout = () => { localStorage.removeItem(TKEY); setToken(""); setCustomer(null); };
